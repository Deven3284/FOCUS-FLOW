...
const TodayTask = () => {
    ...
    }, [filteredTasks, incrementTime]); // Depend on filteredTasks
    ...
    const handleUpdateTasks = () => {
        ...
        Object.keys(edits).forEach(id => {
            updateTask(Number(id), edits[id]);
        });

        setEdits({}); // Clear local edits
        setIsDirty(false); // Reset dirty state
        triggerToast("Tasks Updated ✅");
    };
    ...
    const getStatusParams = (status) => {
        switch (status) {
            case 'Completed': return { color: 'success', icon: <CheckIcon fontSize="small" />, bg: '#dcfce7', text: '#166534' };
            case 'In Progress': return { color: 'primary', icon: <PlayIcon fontSize="small" />, bg: '#dbeafe', text: '#1e40af' };
            default: return { color: 'default', icon: <PendingIcon fontSize="small" />, bg: '#f3f4f6', text: '#374151' };
        }
    };
                                            ...
                                            <TableCell component="th" scope="row" sx={{ width: '30%' }}>
                                                ...
                                                >
                                                    <TextField
                                                        fullWidth
                                                        variant="standard"
                                                        value={task.title.length > 80 ? task.title.slice(0, 80) + '...' : task.title}
                                                        ...
                                                        InputProps={{
                                                            ...
                                                            readOnly: task.title.length > 80
                                                        }}
                                                        inputProps={{
                                                            style: {
                                                                cursor: task.title.length > 80 ? 'pointer' : 'text',
                                                                pointerEvents: task.title.length > 80 ? 'none' : 'auto'
                                                            }
                                                        }}
                                                        sx={{
                                ...
                                >
                                    ...
                                    >
                                        <TextField
                                            ...
                                            InputProps={{
                                                disableUnderline: true,
                                                readOnly: task.title.length > 80 // Prevent inline editing conflict when it's a "button"
                                            }}
                                            inputProps={{
                                                maxLength: 500,
                                                style: { cursor: task.title.length > 80 ? 'pointer' : 'text' }
                                            }}
                                            sx={{
                                                '& .MuiInputBase-input': {
                                                    fontSize: '1.05rem',
                                    ...
                                    {/* Estimated Time */}
                                    <Stack direction="row" spacing={1} sx={{ bgcolor: '#f8fafc', p: 0.5, borderRadius: '10px', border: '1px solid #e2e8f0' }}>
                                        <TextField
                                            type="text"
                                            value={task.estHour}
                                            placeholder="0"
