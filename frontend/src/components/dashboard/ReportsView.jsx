...
const ReportsView = ({ onClose }) => {
    ...
    const [selectedUser, setSelectedUser] = React.useState(''); // Default to Select User
    const [selectedSession, setSelectedSession] = React.useState(null); // For Modal
    const [selectedMonth, setSelectedMonth] = React.useState(new Date().getMonth());
    const [selectedYear, setSelectedYear] = React.useState(new Date().getFullYear());

    React.useEffect(() => {
        // Auto-select removed to allow "Select User" state
    }, []);
    ...
    React.useEffect(() => {
        const fetchHistory = async () => {
            ...
            // Only fetch if Admin is viewing someone (or self) via API to get accurate historical data
            // Or if standard user needs API data? Usually standard user uses `history` from store.
            // But let's unify: If Admin, use API. If User, can use Store OR API.
            // Existing pattern seemed to use API for Admin.

            if (isAdmin) {
                setIsLoadingHistory(true);
                try {
                    const dateStr = `${selectedYear}-${String(selectedMonth + 1).padStart(2, '0')}-01`;
                    // If selectedUser is 'All', pass 'All' or null to API. API handles 'All' or null.
                    const targetUser = selectedUser === 'All' ? 'All' : selectedUser;
                    ...
                    if (response.data && response.data.data) {
                        console.log('API History Fetched:', response.data.data.length, 'records');
        ...
        fetchHistory();
    }, [selectedMonth, selectedYear, selectedUser, role]);

    const groupedData = useMemo(() => {
        const rows = [];
        // Use API history if available (Admin Mode), else Local History (User Mode)
        const sourceHistory = (isAdmin && apiHistory.length >= 0) ? apiHistory : history;
        ...
        safeHistory.forEach((session) => {
            // Normalize userId (API returns 'user', local uses 'userId')
            const normalizedUserId = session.userId || session.user;
            if (!normalizedUserId) return;
            session.userId = normalizedUserId; // Ensure consistent field for later checks

            // Date Filter
            let sessionDate;
            if (session.rawDate) {
                sessionDate = new Date(session.rawDate);
            } else {
            ...
            if (!isNaN(sessionDate.getTime())) {
                if (sessionDate.getMonth() !== selectedMonth || sessionDate.getFullYear() !== selectedYear) {
                    // console.log('Date Filter Mismatch:', { sessionDate, selectedMonth, selectedYear });
                    return;
                }
            } else {
                console.log('Invalid Date:', session.date, session.rawDate);
            }

            // Admin Logic
            if (isAdmin) {
                const sId = String(session.userId);
                // console.log('Processing Session:', { sId, selectedUser });
                ...
                // if (!isValidUser && !isLegacy) return; // Allow all data from API

                if (isLegacy && isDeven) return;
                ...
                if (selectedUser && selectedUser !== 'All') {
                    ...
                    if (!isMatch && !(isLegacy && isDeven)) {
                        // console.log('User Mismatch - Skipping:', { sId, selectedUser });
                        return;
                    }
                }
            } else {
            ...
            if (isAdmin) {
                ...
                userName = sessionUser?.name || (currentUser && String(session.userId) === String(currentUser.id) ? 'You' : 'Unknown');
        ...
        if (today.getMonth() === selectedMonth && today.getFullYear() === selectedYear) {
            ...
            Object.keys(activeTasksByUser).forEach(userId => {
                ...
                if (isAdmin) {
                    ...
                    userName = taskUser?.name || (currentUser && String(userId) === String(currentUser.id) ? 'You' : 'Unknown');
    ...
    }, [history, apiHistory, tasks, role, currentUser, users, selectedUser, selectedMonth, selectedYear, isAdmin]);
            ...
            targetUsers.forEach(user => {
                ...
                for (let day = 1; day <= daysInMonth; day++) {
                    ...
                    const addToMap = (taskList) => {
                        taskList.forEach(t => {
                            const tTitle = t.title || t.task || 'Untitled';
                            const key = t.id ? String(t.id) : (tTitle + (t.priority || ''));
                    ...
                    const taskDetail = allTasks.length > 0
                        ? allTasks.map((t, i) => `${i + 1}. ${t.title || t.task || 'Untitled'}`).join(' • ')
                        : '-';
                                ...
                                <div key={task.id || index} className="bg-white border border-gray-100 p-2 rounded-xl hover:shadow-md transition-shadow">
                                    <div className="flex justify-between items-start mb-3 gap-3">
                                        <h4 className="font-semibold text-gray-800 text-base leading-tight flex-1 break-words">
                                            {task.title || task.task || "No Task Name"}
                                        </h4>
