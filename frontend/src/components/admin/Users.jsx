...
const Users = () => {
    ...
    const { users: localUsers, addUser, updateUser, deleteUser: deleteLocalUser } = useMasterStore();
    ...
    const {
        ...
        successMessage,
        fetchUsers,
        saveUser: saveUserApi,
        deleteUser: deleteUserApi,
        clearMessages
    } = useUsersApiStore();

    // Use API users if available, fallback to local
    ...
    // Fetch users from API on mount and when page/filter changes
    useEffect(() => {
        fetchUsers(page, itemsPerPage, searchTerm, workTypeFilter);
    }, [page, workTypeFilter]);

    // Debounced search
    useEffect(() => {
        const timer = setTimeout(() => {
            fetchUsers(1, itemsPerPage, searchTerm, workTypeFilter);
            setPage(1);
        }, 500);
    ...
    const handleSubmit = async () => {
        ...
        const userData = {
            ...formData,
            _id: isEditing ? editId : undefined
        };

        // Call API to save user
        const result = await saveUserApi(userData);
    ...
    const handleEdit = (user) => {
        // Map API user fields to form fields
        setFormData({
            ...
            workType: user.workType?.toLowerCase() || 'onsite',
    ...
    // API handles filtering and pagination, use users directly
                            ...
                            {isMobile ? (
                                <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2, p: 1 }}>
                                    {paginatedUsers.map((user) => (
                                        <Card key={user._id || user.id} elevation={0} sx={{ borderRadius: 3, border: '1px solid #e2e8f0' }}>
                                            <CardHeader
                                                avatar={
                                                ...
                                                action={
                                                    <Box>
                                                        ...
                                                        <IconButton size="small" onClick={() => handleDelete(user._id || user.id)} disabled={currentUser?.id === (user._id || user.id)} sx={{ color: '#ef4444' }}>
                                ...
                                <TableContainer>
                                    <Table>
                                        ...
                                        <TableBody>
                                            {paginatedUsers.map((user) => (
                                                <TableRow key={user._id || user.id} hover sx={{ '&:last-child td, &:last-child th': { border: 0 }, transition: '0.2s' }}>
                                                    <TableCell>
                                                    ...
                                                    <TableCell>
                                                        <Box>
                                                            ...
                                                            <Typography variant="caption" color="#5f6268ff">{user.plainPassword || '******'}</Typography>
                                                        </Box>
                                                    </TableCell>
                                                    <TableCell>
